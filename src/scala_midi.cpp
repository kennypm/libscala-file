...
namespace scala {
  ...
  degree convert_midi_inner(int note, scala::scale scl, scala::kbm map){
    ...
    if(scale_degree < 0) scale_degree += abs(map.map_size);
    ...
    degree deg = scl.degrees[ map.mapping[scale_degree] ];

    if(note != map.reference_note)
      deg /= convert_midi_inner(map.reference_note, scl, map);

    if(num_octaves > 0)
      for(int i=0; i<num_octaves; i++) deg *= octave_value;
    else
      for(int i=0; i>num_octaves; i--) deg /= octave_value;
